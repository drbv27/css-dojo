...
The server decides whether an exercise was completed, by grading the student's
answer itself, instead of believing the score the browser sent.

**Track relevance**: none — this is **track-agnostic**. It governs
`POST /api/progress`, which every exercise of every track writes through, and
the one grading module both the exercise UI and that route call.

**Written against code that already runs.** This change's spec phase was never
executed while it was being built: `explore` and `propose` were done, the work
shipped in PRs #39, #40 and #41, and the record was left claiming `spec` was
pending. Every requirement below was therefore derived by reading the merged
implementation and its guards, not by planning ahead of them — and each names
where it is enforced, so the claim is checkable rather than assertive. Every
