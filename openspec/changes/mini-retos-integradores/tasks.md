...
- [ ] A.1 `retoPasos?: { instruccion: string; esperado: string }[]` on `Exercise`. **Optional**, and its absence means "an ordinary exercise" — never "a challenge with zero steps".
- [ ] A.2 `cssEsperadoDe(ejercicio)` joins the steps. A challenge MUST NOT hand-write `targetCSS`: two sources of truth for the same fact drift, and the drift shows the student a preview of one thing while grading another.
- [ ] A.3 `calificar` grades a challenge step by step with `compararReglas`, returning per-step verdicts.
- [ ] A.4 **100 or 0, never a fraction.** Instructor decision 2026-08-28. Three of four steps is 75, which clears the route's `score >= 70` and completes a challenge with a whole step skipped — on the exercise whose point is doing the parts together, and which counts toward the certificate.
...
- [ ] A.5 `RetoIntegrador` component: numbered steps beside the editor, each showing pending / satisfied / outstanding. Follows the `JsBehaviorExercise` precedent — `ExerciseRenderer` branches, `LiveEditorExercise` is not touched.
- [ ] A.6 The reference solution renders only when this student's `Progress` for the challenge reads `completed: true`. A lock in the interface, not a boundary — the spec says so and so must the code comment.
- [ ] A.7 Curriculum guards:
...
- [ ] A.8 Write the `box-model` challenge: integrates `box-sizing`, `padding`, `border` and the visible-width calculation in one task.
- [ ] A.9 **Positive control:** give the pilot three of its four steps and confirm it scores **0**, not 75. If it scores 75, A.4 did not land.
- [ ] A.10 **Positive control:** render the challenge for a student with no `Progress` and confirm the reference solution is absent from the output; then with `completed: true` and confirm it appears.
- [ ] A.11 **Positive control:** break `cssEsperadoDe` so the preview and the grading disagree, and confirm a guard turns red. A derived target that nothing checks is a hand-written target with extra steps.
